...
class EventsRepositoryImpl extends EventsRepository {
  ...
  Future<List<UserEvent>> getEvents() async {
    if (await isOnline()) {
      final events = (await cloudDataSource.getAllRows(MDataTable.events) ?? [])
  ...
  Future<Result<Unit, Unit>> postEvent(UserEvent userEvent) async =>
      await cloudDataSource.appendRow(userEvent.toMap(), MDataTable.events)
          ? Result.success(unit)
          : Result.error(unit);

  @override
  Future<void> removeEvent(String eventId) async {
    await cloudDataSource.deleteRow(MDataTable.events, rowKey: eventId);
  ...
  Future<void> markAsSeen(List<UserEvent> events) async {
    for (final event in events) {
      cloudDataSource.updateValue(true, MDataTable.events, rowKey: event.id, columnKey: "seen");
    }
  }
}
